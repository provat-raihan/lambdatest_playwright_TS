  ...
  runTest() {
    test.describe("My account sidebar functionality test", () => {
      test.describe("My account functionality testing while logged out", () => {
        test.beforeEach(
          async ({ runner, envData, homePage, myAccountPage }) => {
        ...
        test("Verify clicking on my account directs to login while the account is not logged in", async ({
          ...
          await runner.clickOnElement(myAccountPage.sidebarOptions.myAccount);
          await runner.verifyUrlContains(envData.loginUrl);
          await runner.verifyElementIsVisible(
            myAccountPage.loginPage.loginPageHeader
          );
      ...
      test.describe("My account tests while logged in", () => {
        ...
        test("verifying my Account items redirect to respective pages ", async ({
          runner,
          myAccountPage,
          envData,
        }) => {
          ...
          await runner.verifyToHaveExactText(
            myAccountPage.myAccount.myAccountHeader,
            myAccountData.myAccount.myAccountHeaderText
          );
          await runner.verifyAnchorLinks(
            myAccountPage.myAccount.myAccountItems,
            [
        ...
        test("verifying my orders items redirect to respective pages ", async ({
          runner,
          myAccountPage,
          envData,
        }) => {
          await runner.verifyElementIsVisible(
            myAccountPage.myAccount.myOrdersHeader
          );
          await runner.verifyToHaveExactText(
            myAccountPage.myAccount.myOrdersHeader,
            myAccountData.myAccount.myOrdersHeaderText
          );
          await runner.verifyAnchorLinks(
            myAccountPage.myAccount.myOrdersItems,
            [
        ...
        test("verifying register affiliate account directs to respective pages ", async ({
          runner,
          myAccountPage,
          envData,
        }) => {
          await runner.verifyElementIsVisible(
            myAccountPage.myAccount.myAffiliateAccountHeader
          );
          await runner.verifyToHaveExactText(
            myAccountPage.myAccount.myAffiliateAccountHeader,
            myAccountData.myAccount.myAffiliateAccountHeaderText
          );
          await runner.clickOnElement(
            myAccountPage.myAccount.registerAffiliateAccount
          );
          ...
          await runner.verifyToHaveExactText(
            myAccountPage.myAccount.myAccountBreadcrumbActive,
            myAccountData.affiliateBreadcrumbText
          );
        });
        test("verifying editing of affiliate account is done with valid data ", async ({
          runner,
          myAccountPage,
          envData,
          fakeUser,
          ...
          await runner.verifyElementIsVisible(
            myAccountPage.registerAffiliateAccountPage
              .AffiliateAccountPageHeader
          );
          await runner.verifyElementIsVisible(
            myAccountPage.registerAffiliateAccountPage.AffiliateAccountLegend
          );
          await runner.fillInputBox(
            myAccountPage.registerAffiliateAccountPage
              .AffiliateAccountPageCompanyField,
            fakeUser.firstName
          );

          await runner.fillInputBox(
            myAccountPage.registerAffiliateAccountPage
              .AffiliateAccountPageWebsiteField,
            fakeUser.lastName
          );
          await runner.verifyElementIsVisible(
            myAccountPage.registerAffiliateAccountPage.paymentInfoLegend
          );
          ...
          await runner.selectRadioOption(
            myAccountPage.registerAffiliateAccountPage.paymentMethodRadioInput,
            myAccountData.registerAffiliateAccountPage.radioButtonOneText
          );
          await runner.fillInputBox(
            myAccountPage.registerAffiliateAccountPage.checkPayeeNameInputField,
            fakeUser.firstName
          );
          await runner.clickOnElement(
            myAccountPage.registerAffiliateAccountPage.continueButton
          );
        });
